...
# 4. Synthetic demo traces — use Claude **arize-synthetic-demo** skill (not in-app). classify-demo + /api/custom-demo/skill hint only.
...
def get_component_for_path(path: str) -> str:
    ...
    if (
        ...
        or path.startswith("/api/custom-demo")
    ):
        return COMPONENT_DEMO  # skill-aligned hints + checklist (no server-side trace synthesis)

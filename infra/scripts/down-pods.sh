...
# Cost when "down": ~$73/month (control plane only). To bring pods back: infra/scripts/up-pods.sh (from repo root).
...
echo "Done. No worker nodes; no EC2 cost. To bring pods back: infra/scripts/up-pods.sh"

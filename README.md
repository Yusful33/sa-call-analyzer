...
| **Synthetic demo (skill)** | **`/api/classify-demo`** (+ optional overrides) classifies Gong/CRM signals and returns a ready-to-paste prompt for the **[arize-synthetic-demo](https://github.com/Arize-ai/solutions-resources/blob/main/.claude/skills/arize-synthetic-demo/SKILL.md)** Claude skill. **`POST /api/generate-demo`** builds the ZIP (`generator.py`, README, etc.). Traces reach Arize when you run **`generator.py`** from that ZIP (see README) **or** when the API has **`DEMO_AUTO_PUSH_TO_ARIZE=1`** plus **`ARIZE_SPACE_ID`** and **`ARIZE_API_KEY`** (server runs `generator.py --count …` after packaging; evals still require the local venv from the ZIP). **`GET /api/custom-demo/skill`** returns static skill pointers. |
